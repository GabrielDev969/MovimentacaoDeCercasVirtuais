...
export class DeviceConsumer {
    ...
    try {
      ...
      if (error instanceof Error) {
        if (error.message.includes('ECONNREFUSED')) {
          throw new Error(
            `Não foi possível conectar ao Kafka: ${error.message}`
          );
        }
        throw error;
      }
  ...
  private async processMessage(message: KafkaMessage): Promise<void> {
    ...
    const requiredFields: (keyof DeviceMessage)[] = [
      'device_id',
      'identifier',
      'latitude',
      'longitude',
    ...
    const transition = stateManager.processTransition(
      device_id,
      geoResult.isInside,
      { identifier: identifier, plate: plate || code || 'N/A' }
    );
    ...
    if (transition.hasTransition) {
      if (transition.duration === null || !geoResult.areaName) {
        console.warn(
          'Dados incompletos para transição - duration ou areaName ausentes'
        );
        return;
      }
